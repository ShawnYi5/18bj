...
class StorageChain(abc.ABC):
    ...
    def __init__(self, storage_reference_manager, caller_name: str, name_prefix: str, timestamp=None):
    ...
    def _query_key_storage_info_list(self):
        ...
        for i in range(storage_info_list_len):
            ...
            assert storage_info['status'] != m.SnapshotStorage.STATUS_DELETED
            ...
            if storage_info['image_path'] != self._storage_info_list[i + 1]['image_path']:
                ...
                continue
            if self._storage_info_list[i + 1]['status'] in m.SnapshotStorage.STATUS_WRITING:
    ...
    def insert_head(self, storage_obj):
        assert not self._valid
        self._storage_info_list.insert(0, storage_obj)

    def insert_tail(self, storage_obj):
        assert not self._valid
        self._storage_info_list.append(storage_obj)
...
class StorageChainForRead(StorageChain):
    ...
    def __init__(self, storage_reference_manager, caller_name: str, timestamp=None):
        super(StorageChainForRead, self).__init__(storage_reference_manager, caller_name, 'r', timestamp)
    ...
    def acquire(self):
        assert self._storage_info_list[-1]['status'] != m.SnapshotStorage.STATUS_CREATING
        try:
