...
class DiskSnapshotStorageTree(object):
    ...
    def create_tree_inst(tree_ident):
        ...
        storage_objs = storage.SnapshotStorageTreeQuery(tree_ident).valid_obj_dicts()
    ...
    def get_node_by_ident(self, ident):
        ...
        return find(
            self.root_node,
            lambda node: node.storage['ident'] == ident if 'ident' in node.storage else node.storage[
                                                                                            'new_ident'] == ident)
